...
export default function MySpacePage() {
  ...
  return (
    ...
    >
      ...
      {/* Main Content Area */}
      <div className="flex-1 flex items-start justify-start p-4 md:p-8 pt-16 pb-32">
        <div className="w-full max-w-md space-y-4">
          {/* Ibn al-Qayyim Card */}
          <div className="transition-all duration-300">
            <Card className={`bg-card/70 backdrop-blur-md border-border/50 transition-all duration-300 py-0 gap-0 ${
                ...
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <CardTitle className={`font-medium transition-all ${
                      isQuoteMinimized ? 'text-xs text-muted-foreground' : 'text-xs'
                    }`}>
              ...
              {!isQuoteMinimized && (
                <CardContent className="pt-0 px-3 pb-3">
                  <blockquote className="text-xs text-foreground/90 italic leading-relaxed border-l-2 border-primary/30 pl-3">
                  ...
                  <p className="text-xs text-muted-foreground mt-0.5 text-right opacity-75">
          ...
          <div className="transition-all duration-300">
            <Card className="bg-card backdrop-blur-sm opacity-100 py-0 gap-0">
              <CardHeader className="pb-1.5 px-3 pt-3">
                <div className="flex items-center justify-between gap-2">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <CardTitle className="text-xs md:text-sm">Quran recitation</CardTitle>
                  </div>
                  <Button
                    variant="ghost"
                    size="sm"
              ...
              <CardContent className="pt-1.5 px-3 pb-3">
                <div className={`w-full rounded-lg overflow-hidden transition-all duration-300 ${isMinimized ? 'aspect-square h-32' : 'aspect-square'}`}>
                  <iframe
                    width="100%"
                    height="100%"
                    src={youtubeEmbedUrl}
